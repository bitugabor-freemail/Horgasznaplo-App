...
class AdatTarolo {
  ...
  // ÚJ: Folyamatos, streamelt ZIP kódoló a progress bar-hoz
  static Future<String> letrehozTeljesExportZIPFajl(Function(double) onProgress) async {
    ...
    // 1. JSON mentése átmeneti fájlba
    final jsonStr = await letrehozExportJson();
    ...
    // 3. Fájlok egyenkénti becsomagolása és UI frissítése
    if (osszesFajl == 0) {
      onProgress(1.0);
    } else {
      for (var f in csomagolandoFajlok) {
        ...
        // EZ A VARÁZSLAT: Hagyunk időt a Flutternek megrajzolni a betöltőcsíkot!
        await Future.delayed(const Duration(milliseconds: 2)); 
      }
    }

    zipEncoder.close();
    if (await jsonFile.exists()) await jsonFile.delete(); // Takarítás
